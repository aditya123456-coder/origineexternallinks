...
export async function isBalancedTree(userCode, testCase, currentlang) {
    let lang = LANGUAGE_CONFIG[currentlang];
    let fullcode = null;

    const treeArr = JSON.stringify(testCase.tree).replace(/null/g, "null");
    // Use -9999 as null marker for C/CPP
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Integer[] arr = ${treeArr};
        TreeNode root = buildTree(arr);
        System.out.print(sol.isBalanced(root) ? 1 : 0);
    }
}`;
    }
...
console.log(isBalanced(root) ? 1 : 0);
`;
    }
