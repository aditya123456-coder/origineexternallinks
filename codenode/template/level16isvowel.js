export function runAndValidate({ dropzone, output, validationResult, levelConfig }) {
  ...
  // FINAL CHECK
  if (!foundPrints.includes('vowel')) {
    output.innerHTML += `<span class="error">❌ Missing: vowel output</span><br>`;
    hasError = true;
  }

  if (!foundPrints.includes('consonant')) {
    output.innerHTML += `<span class="error">❌ Missing: consonant output</span><br>`;
    hasError = true;
  }
